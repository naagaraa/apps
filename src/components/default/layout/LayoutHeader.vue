...
const classSidebarBase = computed(() => {
    if (!isLoggin.value) {
        return 'left-0 sm:left-0'
    } else if (isLoggin.value && minibar.value) {
        return 'left-0 sm:left-16'
    } else if (isLoggin.value && !minibar.value) {
        return 'left-0 sm:left-56'
    }
})
...
<template>
    ...
    <nav :class='classSidebarBase'
        ...
        <div class="px-3 py-3 lg:px-5 lg:pl-3">
            <div class="flex md:items-center justify-start md:justify-between">
                <TopLogoActive />
                <TopNavigation />
                <SearchInput />
                <div v-if="isLoggin">
